...
MoveLookController::MoveLookController(_In_ CoreWindow const& window, _In_ CoreDispatcher const& dispatcher) :
...
MoveLookController::MoveLookController(_In_ CoreWindow const& window) :
...
void MoveLookController::InitWindow(_In_ CoreWindow const& window)
{
    ...
    SystemNavigationManager::GetForCurrentView().BackRequested({ this, &MoveLookController::OnBackRequested });
...
winrt::fire_and_forget MoveLookController::ShowCursor()
{
...
winrt::fire_and_forget MoveLookController::HideCursor()
{
    if (m_dispatcher != nullptr)
    {
        // Use the CoreDispatcher to ensure that the pointer is updated on the UI thread.
...
void MoveLookController::OnGamepadAdded(_In_ IInspectable const& /* sender */, _In_ Gamepad const& /* gamepad */)
{
...
void MoveLookController::OnGamepadRemoved(_In_ IInspectable const& /* sender */, _In_ Gamepad const& /* gamepad */)
{
    // OnGamepadAdded and OnGamepadRemoved can be called from a worker thread. For simplicity,
    // defer updating the active gamepad until Update().
    m_gamepadsChanged = true;
